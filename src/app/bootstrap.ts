import { commentList, data, options } from '../modules/document/document.mock'
...
import { formatPrismToken } from '../shared/utils/prism.utils'
import { Signature } from '../shared/components/signature/Signature'
import { debounce, nextTick, scrollIntoView } from '../shared/utils/common.utils'
...
window.onload = function () {
  ...
  Reflect.set(window, '__EDITOR_INSTANCE__', instance)

  // Destroy menu popups on outside click
  ...
  // 3. | Font | Increase Size | Decrease Size | Bold | Italic | Underline | Strikeout | Superscript | Subscript | Font Color | Highlight |
  ...
  // 4. | Table | Image | Hyperlink | Separator | Watermark | Code Block | Divider | Control | Checkbox | LaTeX | Date Picker
  ...
  function recoveryTable() {
    // Restore selection style, title, selected rows/cols
        ...
        new Dialog({
          ...
          data: [
            ...
            },
            {
              ...
              required: true,
              height: 100,
              placeholder: `Enter value set JSON, e.g.:\n[{\n"value":"Yes",\n"code":"001"\n}]`
            }
          ],
          onConfirm: payload => {
            const placeholder = payload.find(
          ...
          data: [
            {
              ...
              label: 'Default Value',
              name: 'code',
              placeholder: 'Enter default values, separated by commas'
            },
            {
              type: 'textarea',
              label: 'Value Set',
    ...
    new Dialog({
      ...
      data: [
        ...
        },
        {
          ...
          label: 'Width',
          name: 'width',
          placeholder: 'Enter width (defaults to page width)'
        },
        {
          type: 'number',
          label: 'Height',
        ...
        },
        {
          ...
          name: 'srcdoc',
          required: false,
          placeholder: 'Enter HTML code (website type only)'
        }
      ],
      onConfirm: payload => {
        ...
        if (!height) return
        // At least one of src or srcdoc must be provided
  ...
  // 6. Catalog | Page Mode | Paper Scale | Paper Size | Paper Direction | Margins | Fullscreen | Settings
      ...
      data: [
        {
          ...
          placeholder: 'Enter editor configuration JSON'
        }
      ],
      onConfirm: payload => {
  ...
  modeElement.onclick = function () {
    ...
    // Update menu bar visual permissions
  ...
  async function updateComment() {
    ...
    for (const comment of commentList) {
      ...
      // Check if group id exists in editor
      if (groupIds.includes(comment.id)) {
        // Append if dom does not exist
        if (!activeCommentDom) {
        ...
        // Remove dom if group id not found in editor
          ...
          data: [
            {
              ...
              name: 'value',
              required: true,
              placeholder: 'Enter caption text, use {imageNo} for image number'
            }
          ],
          onConfirm: payload => {
            const value = payload.find(p => p.name === 'value')?.value
